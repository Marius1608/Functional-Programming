...
var $author$project$Cursor$fromList = function (xs) {
	if (xs.b) {
		...
		var y = xs.b;
		return $elm$core$Maybe$Just(
			A3($author$project$Cursor$Cursor, _List_Nil, x, y));
	} else {
		return $elm$core$Maybe$Nothing;
	}
};
...
var $author$project$Util$Time$durationBetween = F2(
	function (t1, t2) {
		var ms2 = $elm$time$Time$posixToMillis(t2);
		var ms1 = $elm$time$Time$posixToMillis(t1);
		if (_Utils_cmp(ms2, ms1) < 1) {
			return $elm$core$Maybe$Nothing;
		} else {
			var diffMs = ms2 - ms1;
			var diffSeconds = (diffMs / 1000) | 0;
			var remainingSeconds1 = diffSeconds % ((24 * 60) * 60);
			var hours = (remainingSeconds1 / (60 * 60)) | 0;
			var remainingSeconds2 = remainingSeconds1 % (60 * 60);
			var minutes = (remainingSeconds2 / 60) | 0;
			var seconds = remainingSeconds2 % 60;
			var days = (diffSeconds / ((24 * 60) * 60)) | 0;
			return $elm$core$Maybe$Just(
...
var $author$project$Util$Time$formatDuration = function (duration) {
	var formatTimeUnit = F2(
		function (count, unit) {
			return (count === 1) ? ($elm$core$String$fromInt(count) + (' ' + unit)) : ($elm$core$String$fromInt(count) + (' ' + (unit + 's')));
		});
	var parts = A2(
		...
		_List_fromArray(
			[
				(duration.days > 0) ? $elm$core$Maybe$Just(
				A2(formatTimeUnit, duration.days, 'day')) : $elm$core$Maybe$Nothing,
				(duration.hours > 0) ? $elm$core$Maybe$Just(
				A2(formatTimeUnit, duration.hours, 'hour')) : $elm$core$Maybe$Nothing,
				(duration.minutes > 0) ? $elm$core$Maybe$Just(
				A2(formatTimeUnit, duration.minutes, 'minute')) : $elm$core$Maybe$Nothing,
				(duration.seconds > 0) ? $elm$core$Maybe$Just(
				A2(formatTimeUnit, duration.seconds, 'second')) : $elm$core$Maybe$Nothing
			]));
					...
					[
						...
						function (str) {
							...
							if (_v0.$ === 'Just') {
								...
								return $author$project$Model$ConfigChanged(
									$author$project$Model$PostsConfig$ChangePostsToShow(config.postsToShow));
							}
						})
					]),
				_List_fromArray(
					...
					[
						...
						function (str) {
							...
							if (_v1.$ === 'Just') {
								...
								return $author$project$Model$ConfigChanged(
									$author$project$Model$PostsConfig$ChangeSortBy(config.sortBy));
							}
						})
					]),
				A2(
					...
					[
						...
						_List_fromArray(
							[
								...
								function (checked) {
									return $author$project$Model$ConfigChanged(
										$author$project$Model$PostsConfig$ChangeShowJobs(checked));
								})
							]),
						_List_Nil),
					...
					[
						...
						_List_fromArray(
							[
								...
								function (checked) {
									return $author$project$Model$ConfigChanged(
										$author$project$Model$PostsConfig$ChangeShowTextOnly(checked));
								})
							]),
						_List_Nil),
